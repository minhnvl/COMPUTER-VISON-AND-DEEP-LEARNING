...
class DataBowl3Detector(Dataset):
    def __init__(self, split, config, phase = 'train',split_comber=None):
        ...
        data_dir_label = data_dir.replace("_img","_label")
        #########################
...
#        idcs = split
        ...
        for idx in idcs:
            print(idx)
            idx = idx.replace("_label.npy","")
            if config['luna_raw'] ==True:
                try:
                    l = np.load(os.path.join(data_dir_label, '%s_label_raw.npy' %idx))
                except:
                    l = np.load(os.path.join(data_dir_label, '%s_label.npy' %idx))
            else:
                l = np.load(os.path.join(data_dir_label, '%s_label.npy' %idx))
        labels.append(l)
